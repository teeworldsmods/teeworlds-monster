...
void CNetBan::MakeBanInfo(const CBan<T> *pBan, char *pBuf, unsigned BuffSize, int Type) const
{
	...
	if(pBan->m_Info.m_Expires != CBanInfo::EXPIRES_NEVER)
	{
		...
		int Seconds = pBan->m_Info.m_Expires - time_timestamp(), Hours = 0, Days = 0;
		if(Mins >= 60)
        {
            Hours = Mins/60;
            Mins %= 60;
		...
		char aDay[64];
		char aMin[128];
		str_format(aMin, sizeof(aMin), "%d minute%s (%d second%s left)", Mins, Mins <= 1 ? "" : "s", Seconds, Seconds > 1 ? "s" : "");
		if(Hours || Days)
            str_format(aHour, sizeof(aHour), "%d hour%s and ", Hours, Hours <= 1 ? "" : "s");
        if(Days)
            str_format(aDay, sizeof(aDay), "%d day%s, ", Days, Days == 1 ? "" : "s");
        char Buffer[512];
        str_format(Buffer, sizeof(Buffer), "%s%s%s", Days ? aDay : "", (Hours || Days) ? aHour : "", aMin);
		str_format(pBuf, BuffSize, "%s for %s (%s)", aBuf, Buffer, pBan->m_Info.m_aReason);
	}
	else
